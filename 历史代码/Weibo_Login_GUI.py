...
class WeiboLoginApp:
    ...
    def setup_ui(self):
        ...
        # 标题
        title_label = ttk.Label(main_frame, text="微博扫码登录工具", font=("Arial", 18, "bold"))
        ...
        self.clear_cookies_btn.grid(row=3, column=0, pady=5, sticky=(tk.W, tk.E))
        
        # 状态显示
        status_frame = ttk.LabelFrame(control_frame, text="状态信息", padding="5")
        status_frame.grid(row=4, column=0, sticky=(tk.W, tk.E), pady=(20, 0))
        
        self.login_status_label = ttk.Label(status_frame, text="登录状态: 未登录", foreground="red")
        self.login_status_label.grid(row=0, column=0, pady=5)
        
        # 进度条
        self.progress = ttk.Progressbar(control_frame, mode='indeterminate')
        self.progress.grid(row=5, column=0, pady=20, sticky=(tk.W, tk.E))
        
        # 右侧显示区域 (二维码)
        display_frame = ttk.LabelFrame(main_frame, text="登录二维码", padding="10")
        display_frame.grid(row=1, column=1, sticky=(tk.W, tk.E, tk.N, tk.S))
        display_frame.columnconfigure(0, weight=1)
        display_frame.rowconfigure(0, weight=1)
        
        self.qr_label = ttk.Label(display_frame, text="点击'获取登录二维码'开始\n\n如果没有安装selenium，\n请手动在浏览器中打开登录页面", 
    ...
    def _fetch_qr_with_selenium(self):
        ...
        try:
            ...
            if Service:
                ...
                # Selenium 3 兼容模式：尝试通过修改 subprocess.Popen 隐藏黑框
                if os.name == 'nt':
